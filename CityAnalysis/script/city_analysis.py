...
CONFIG_FILE = os.path.join(BASE_DIR, "script", "script_config")
...
if not files:
    raise SystemExit(f"No city JSON file found in {INPUT_DIR}")
...
def extract_age_from_townhall_id(cityentity_id: str) -> Optional[str]:
...
current_age_no_space: Optional[str] = None
...
def entity_troop_info(entity: dict, age_key: Optional[str]) -> Tuple[Dict[Tuple[str, str], int], bool]:
...
def entity_goods_info(entity: dict, age_key: Optional[str]) -> Tuple[Dict[str, int], bool]:
    """
...
summary_file = os.path.join(OUTPUT_DIR, f"city_summary_{timestamp}.txt")
