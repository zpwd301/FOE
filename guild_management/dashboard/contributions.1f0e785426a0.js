...
(function () {
  ...
  function groupDetailRows(rows, labelIndex, absolute = false) {
    const groups = new Map();
    rows.forEach((row) => {
      const amount = absolute ? Math.abs(Number(row[5])) : Number(row[5]);
      groups.set(row[labelIndex], (groups.get(row[labelIndex]) || 0) + amount);
    });
    return [...groups.entries()].sort((a, b) => b[1] - a[1] || String(a[0]).localeCompare(String(b[0])));
  }
  ...
  function renderProductionRecords(rows) {
    const memberRecords = detailRecordsInPeriod(rows).filter((row) => Number(row[5]) > 0);
  ...
  function renderUsageDetails(rows) {
    const usageRecords = detailRecordsInPeriod(rows).filter((row) => Number(row[5]) < 0);
    ...
    if (usageRecords.length) {
      const usageTotal = usageRecords.reduce((total, row) => total + Math.abs(Number(row[5])), 0);
      $("#member-usage-total").textContent = `−${fmt.format(usageTotal)}`;
      $("#member-usage-count").textContent = plural(usageRecords.length, "usage record");
      $("#member-usage-purpose-list").innerHTML = renderBreakdown(groupDetailRows(usageRecords, 6, true), usageTotal, (value) => value, true);
      $("#member-usage-goods-list").innerHTML = renderBreakdown(groupDetailRows(usageRecords, 4, true), usageTotal, (value) => value, true);
      $("#member-usage-era-list").innerHTML = renderBreakdown(groupDetailRows(usageRecords, 3, true), usageTotal, eraName, true);
    }
  }
  ...
  async function showProtectedDetails(playerId) {
    ...
    try {
      ...
      renderProductionRecords(detail.records);
      renderUsageDetails(detail.records);
      loading.hidden = true;
