...
Levels 1–201 remain the sourced values. Levels 202–301 are generated by three separately fitted reward families:

```text
FP P1 = round-to-nearest-5(era factor × (level^1.2 - 1) / 3.2)
era factor = era id + 9; No Age uses 14

medal P1 = round(era scale × (level^1.2011 - 1))
blueprint P1–P5 = round(position scale × level^0.8)
```

Each medal era scale and each blueprint position scale is a least-squares fit over the sourced values. P2–P5 FP and medals continue to use the rules above; they are not fitted independently.

The FP equation was tested against every sourced level 11–201 value: all predictions were within 5 FP. Rolling holdouts inside the 1–201 range gave a mean absolute percentage error of about 0.014% for medal P1, with a worst case near 0.11%. The same rolling test reproduced 94% of blueprint cells exactly and every cell within one blueprint. These are estimates, so the UI labels levels 202–301 as modeled.

As a separate out-of-sample check after fitting, 450 available level 202–251 P1 rows across seven eras were compared with FoE Helper's public Legendary Building response. FP was within 5 in every row; medal P1 had 0.0096% mean absolute percentage error; and all 2,250 blueprint position values were within one blueprint. None of those later values is used to fit the checked-in model.
...
The newer helper also fills the earlier Industrial Age and Space Age Venus FP gaps present in Forge Hammer 1.6.0. The checked-in dataset has sourced FP, medal, and blueprint coverage through target level 201 for all 49 Great Buildings, plus the labeled model extension through target level 301.
